(() => {
  ...
  var init_esm2 = __esm({
    "node_modules/@aparajita/capacitor-biometric-auth/dist/esm/index.js"() {
      ...
      init_definitions();
      init_web_utils();
      proxy = registerPlugin("BiometricAuthNative", {
        web: async () => {
  ...
  var init_esm3 = __esm({
    "node_modules/@aparajita/capacitor-secure-storage/dist/esm/index.js"() {
      init_dist();
      init_definitions3();
      proxy2 = registerPlugin("SecureStorage", {
        web: async () => {
    ...
    "bridge/sd-auth-source.js"() {
      ...
      (function() {
        ...
        async function isFingerprintEnabled() {
          try {
            const value = await proxy2.getItem(STORAGE_KEY_ENABLED);
            return value === "true";
          } catch (e) {
            return false;
          }
        ...
        async function saveCredentials(username, password) {
          await proxy2.setItem(STORAGE_KEY_USERNAME, username);
          await proxy2.setItem(STORAGE_KEY_PASSWORD, password);
          await proxy2.setItem(STORAGE_KEY_ENABLED, "true");
        }
        async function getSavedCredentials() {
          const username = await proxy2.getItem(STORAGE_KEY_USERNAME);
          const password = await proxy2.getItem(STORAGE_KEY_PASSWORD);
          return { username, password };
        }
        async function clearCredentials() {
          try {
            await proxy2.removeItem(STORAGE_KEY_USERNAME);
          } catch (e) {
          }
          try {
            await proxy2.removeItem(STORAGE_KEY_PASSWORD);
          } catch (e) {
          }
          try {
            await proxy2.removeItem(STORAGE_KEY_ENABLED);
          } catch (e) {
          }
        }
        async function isPurchaseFingerprintEnabled() {
          try {
            const value = await proxy2.getItem(STORAGE_KEY_PURCHASE_ENABLED);
            return value === "true";
          } catch (e) {
            return false;
          }
        ...
        async function enablePurchaseFingerprint(pin) {
          await proxy2.setItem(STORAGE_KEY_PURCHASE_PIN, pin);
          await proxy2.setItem(STORAGE_KEY_PURCHASE_ENABLED, "true");
        }
        async function getStoredPurchasePin() {
          try {
            return await proxy2.getItem(STORAGE_KEY_PURCHASE_PIN);
          } catch (e) {
            return null;
          }
        }
        async function clearPurchasePin() {
          try {
            await proxy2.removeItem(STORAGE_KEY_PURCHASE_PIN);
          } catch (e) {
          }
          try {
            await proxy2.removeItem(STORAGE_KEY_PURCHASE_ENABLED);
          } catch (e) {
          }
        }
